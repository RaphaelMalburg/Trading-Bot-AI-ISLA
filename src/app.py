...
def maybe_record_trade(run_id: int, run: dict) -> None:
    ...
    try:
        ...
        logger.info("Trade recorded in ledger: order_id=%s run_id=%s", run["order_id"], run_id)
...
def backtest_page():
    ...
    try:
        ...
        if os.path.isdir(models_dir):
            pngs = sorted(
                [f for f in os.listdir(models_dir) if f.startswith("dream_dynamic") and f.endswith(".png")],
...
def api_kill_switch():
    ...
    try:
        ...
        latest = get_latest()
        last_close = float(latest.get("btc_close")) if latest and latest.get("btc_close") else 0.0
...
def api_trades_csv():
    ...
    headers = ["entry_time", "entry_price", "exit_time", "exit_price",
               "exit_reason", "pnl_dollars", "pnl_percent", "duration_hours", "qty"]
...
def api_runs_csv():
    ...
    headers = ["timestamp", "btc_close", "prediction_label", "confidence",
               "sentiment_score", "action", "stop_loss", "take_profit",
               "position_qty", "leverage", "error"]
...
def tp_watchdog_loop():
    ...
    while True:
        try:
            ...
            positions = trading_client.get_all_positions()
            btc_pos = next((p for p in positions if p.symbol.replace("/", "").upper() == "BTCUSD"), None)
            if not btc_pos:
                continue
            ...
            logger.warning("TP watchdog: %.2f >= %.2f. Closing position.", current_price, tp_target)
